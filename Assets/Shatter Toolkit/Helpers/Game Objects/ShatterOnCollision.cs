    ...
    public class ShatterOnCollision : MonoBehaviour
    {
        ...
        public void OnCollisionEnter(Collision collision)
        {
            if (timeSinceInstantiated >= cooldownTime)
            {
                ...
                {
                    if (!collision.gameObject.CompareTag("cubeClone")&&collision.collider.CompareTag("weapon"))
                    {
        ...
        private IEnumerator delay(GameObject obj)
        {
            yield return new WaitForSeconds(2f);
            obj.SetActive(false);
            /* Set parent Clone */
            obj.transform.SetParent(GameManager.instance.cubeObjects.transform);
        }
